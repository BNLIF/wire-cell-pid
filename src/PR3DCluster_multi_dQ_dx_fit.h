void WCPPID::PR3DCluster::dQ_dx_multi_fit(WCPPID::Map_Proto_Vertex_Segments& map_vertex_segments, WCPPID::Map_Proto_Segment_Vertices& map_segment_vertices, std::map<int,std::map<const WCP::GeomWire*, WCP::SMGCSelection > >& global_wc_map, std::map<std::pair<int,int>,  std::tuple<double, double, int > >& map_2D_ut_charge, std::map<std::pair<int,int>, std::tuple<double, double, int> >& map_2D_vt_charge, std::map<std::pair<int,int>,std::tuple<double, double, int> >& map_2D_wt_charge, double flash_time , double dis_end_point_ext , bool flag_dQ_dx_fit_reg ){
  ...
  // doc sbnd_xin/docs/pr/108 sec 9 -- dQ/dx SYSTEM dump (debug only, env WCP_DQDX_DUMP=<path>; unset => no code path).  Same record layout as the toolkit's WCT_DQDX_DUMP.
  {
    static FILE* s_dq = (getenv("WCP_DQDX_DUMP") ? fopen(getenv("WCP_DQDX_DUMP"), "a") : (FILE*)0);
    if (s_dq) {
      Eigen::VectorXd bu = RUT * MU * data_u_2D, bv = RVT * MV * data_v_2D, bw = RWT * MW * data_w_2D;
      fprintf(s_dq, "%d dqdx call n3d=%d n2d=%d/%d/%d iters=%ld err=%.6g\n", g_wcp_traj_call, (int)n_3D_pos, (int)n_2D_u, (int)n_2D_v, (int)n_2D_w, (long)solver.iterations(), (double)solver.error());
            for (int k = 0; k < A.outerSize(); ++k)
                for (Eigen::SparseMatrix<double>::InnerIterator it(A, k); it; ++it)
                    fprintf(s_dq, "%d A %ld %ld %.10g\n", g_wcp_traj_call, (long)it.row(), (long)it.col(), it.value());
            for (int i = 0; i < (int)n_3D_pos; ++i) fprintf(s_dq, "%d b %d %.10g %.10g\n", g_wcp_traj_call, i, b(i), pos_3D_init(i));
      for (int i = 0; i < (int)n_3D_pos; ++i) {
        ...
        fprintf(s_dq, "%d dqdx %d %.4f %.4f %.4f %.4f %d %d %d %d %.4g %.4g %d %.4g %.4g %d %.4g %.4g %.4g %.1f %zu",
                g_wcp_traj_call, i, traj_pts.at(i).x/units::cm, traj_pts.at(i).y/units::cm, traj_pts.at(i).z/units::cm, local_dx.at(i)/units::cm,
                (int)reg_flag_u.at(i), (int)reg_flag_v.at(i), (int)reg_flag_w.at(i),
                cu, su, bu(i), cv, sv, bv(i), cw, sw, bw(i), fdiag, pos_3D(i), connected_vec.at(i).size());
        for (size_t j = 0; j < connected_vec.at(i).size(); ++j)
          fprintf(s_dq, " %d:%.2f/%.2f/%.2f:%.4g", connected_vec.at(i).at(j), overlap_u.at(i).at(j), overlap_v.at(i).at(j), overlap_w.at(i).at(j), FMatrix.coeff(i, connected_vec.at(i).at(j)));
        fprintf(s_dq, "\n");
      }
      fflush(s_dq);
    }
